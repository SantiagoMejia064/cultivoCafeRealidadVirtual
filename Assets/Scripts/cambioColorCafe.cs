...
public class cambioColorCafe : MonoBehaviour
{
    ...
    {
        if (other.CompareTag("TriggerArea") && !haCambiadoMaterial) // Verifica si el objeto est� entrando en un �rea espec�fica (con el tag "TriggerArea")
        {
    ...
    IEnumerator CambiarMaterial()
    {
        ...
        // Cambiar el material a uno nuevo si no ha cambiado a�n
        if (rend != null && !haCambiadoMaterial)
        {
            rend.material = materialNuevo;  // Cambiar al nuevo material
            haCambiadoMaterial = true; // Evitar que se cambie m�s de una vez
        }
    }
}
